...
const CSP_PROFILES: Record<string, string> = {
  // 主窗口：需要 API 连接、图片、字体（KaTeX）、iframe（artifacts）、Monaco Editor workers
  'index.html':
    "default-src 'self'; connect-src 'self' ws://127.0.0.1:* http://127.0.0.1:*; img-src 'self' data: file: http://127.0.0.1:*; style-src 'self' 'unsafe-inline'; script-src 'self' 'unsafe-eval' 'unsafe-inline'; font-src 'self' data:; frame-src blob: data: file: http://127.0.0.1:* http://localhost:*; worker-src 'self' blob:",
...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      ...
      output: {
        // Monaco Editor 把语言包拆成独立 chunk，避免初始 bundle 过大
        manualChunks: {
