...
export function switchTab(tab: TabType) {
  ...
  // 进入 coding 模式：关闭预览面板，确保展开右侧工作区栏
  if (tab === 'coding') {
    if (s.previewOpen) {
      // 触发 closePreview 不行（需要 store action），直接 setState
...
function buildTabList(pluginPages: PluginPageInfo[], tabOrder: string[]): TabType[] {
  ...
  // chat 与 coding 都是固定 tab，不可拖拽不可关闭
  return ['chat' as TabType, 'coding' as TabType, ...ordered];
}

function getTabLabel(tab: TabType, pluginPages: PluginPageInfo[], locale: string): string {
  if (tab === 'chat') return t('channel.chatTab');
  if (tab === 'coding') return t('channel.codingTab');
...
export function ChannelTabBar() {
  ...
  // chat and coding are always first/second and not draggable; split into visible and overflow
  ...
  const visibleTabs: TabType[] = ['chat' as TabType, 'coding' as TabType, ...visibleDraggable];
  ...
  const onDragStart = useCallback((e: React.DragEvent, tab: TabType) => {
    if (tab === 'chat' || tab === 'coding') { e.preventDefault(); return; }
