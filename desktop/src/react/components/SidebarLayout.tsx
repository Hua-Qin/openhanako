...
import { CHAT_MIN_WIDTH, CODING_MIN_WIDTH } from '../layout-constants';
...
export function updateLayout(): void {
  ...
  const isCoding = currentTab === 'coding';
  const minWidth = isCoding ? CODING_MIN_WIDTH : CHAT_MIN_WIDTH;
  const leftW = s.sidebarOpen ? getSidebarWidth() : 0;
  const rightW = s.jianOpen ? getJianWidth() : 0;
  // 编程模式不显示 previewPanel（编辑器本身就是预览）
  const previewW = !isCoding && currentTab === 'chat' && s.previewOpen ? getPreviewWidth() : 0;
  ...
  if (contentW < minWidth) {
    if (s.jianOpen) {
      ...
      if (newContentW < minWidth && s.sidebarOpen) {
    ...
    if (s.sidebarAutoCollapsed) {
      ...
      if (w - rightW - previewW - channelInspectorW - neededForLeft >= minWidth) {
    ...
    if (s2.jianAutoCollapsed) {
      ...
      if (w - leftW2 - previewW - channelInspectorW - neededForRight >= minWidth) {
