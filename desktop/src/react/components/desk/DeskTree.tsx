...
function TreeNode({
  ...
  onCancelRename: () => void;
  onFileOpen?: (file: DeskFile, subdir: string) => void;
}) {
  ...
  const handleClick = useCallback((event: React.MouseEvent) => {
    ...
    // onFileOpen 模式下：单击文件直接打开（不论 web 或 mount）
    if (!file.isDir && onFileOpen && !multi && !event.shiftKey) previewFile();
    if (!file.isDir && !onFileOpen && (isWebRuntime() || deskWorkspaceMountId) && !multi && !event.shiftKey) previewFile();
  }, [deskWorkspaceMountId, file.isDir, onFileOpen, onSelect, previewFile, subdir, toggleFolder]);
  ...
  // 上面依赖列表已包含 previewFile，previewFile 内已使用 onFileOpen
      ...
      items: [
        {
          ...
          action: () => {
            if (file.isDir) {
              ...
              // onFileOpen 模式下：总是走外部回调（编程模式编辑器打开）
              if (onFileOpen || isWebRuntime() || !nativeRootDir) previewFile();
  ...
  }, [deskBasePath, deskWorkspaceMountId, expandedPaths, file.isDir, file.name, getDragEntries, nativeRootDir, onBeginRename, onFileOpen, onSelect, onShowMenu, onStartCreate, previewFile, selectedPaths, setDeskExpandedPaths, subdir, treeFilesByPath]);
